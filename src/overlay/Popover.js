...
import {uuid, create, on, stopPropagation, off, setStyle} from '../utils';
...
class Popover extends ol.Overlay {
  constructor (options = {}) {
    ...
    const container = create('div', _className + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE);
    ...
    if (options['showMinimize']) {
      ...
      on(minimize, 'click', this.showMinimize, this);
    }
  ...
  show (coordinates, html, options = {}) {
    ...
    if (this.markFeature) {
      let size = this.markFeature.getStyle().getImage().getSize() || options['size'] || [13, 28];
