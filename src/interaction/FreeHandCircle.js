...
class FreeHandCircle extends ol.interaction.Pointer {
  /**
   * 处理移动事件
  ...
  constructor (options = {}) {
     ...
     * @type {*}
     */
    this.layerName = options['layerName'] || 'FREE_HAND_CIRCLE';

    /**
     * 中心点样式
     * @type {*}
     */
    this.centerStyle = options['centerStyle'] || null;
  ...
  initDrawInteraction () {
    ...
    let style_ = StyleFactory.getStyle(this.style_);
  ...
  createCircle (center, radius) {
    ...
    if (!this.circleFeature) {
      ...
      let _uuid = (this.draw && this.draw.get('uuid')) ? this.draw.get('uuid') : uuid();
   ...
   * @private
   */
  transformCenterAndRadius_ (center, radius) {
   ...
   * @private
   */
  addLabelFeature_ (coordinates, type) {
    if (type === 'center') {
      if (!this.centerFeature) {
        ...
        if (this.centerStyle) {
          let _style = StyleFactory.getStyle(this.centerStyle);
      ...
      if (!this.labelFeature) {
        ...
        let _style = StyleFactory.getStyle({
          image: {
            type: 'icon',
            image: {
   ...
   * @private
   */
  drawTextLabel_ (text, coordinates) {
    if (!this.textOverlay) {
